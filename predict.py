...
class Predict:
    ...
    def run(self):
        ...
        paths = paths + [mask for mask in glob.glob(os.path.join(self.parameters.dest + '/temp/resampled', '*.tif')) if 'watermask' in mask]
    ...
    def predmlr(self, pathxmodel: str, Xarray: list, array: dict, reference: str, dest:str) -> None:
        ...
        for y_band in self.parameters.OLCIBAND:
            ...
            ssamasked = ssar * np.where(array['watermask'] == 1, 1, np.nan) # it is used to mask water pixels.                                                                                     # It is necessary because the image does not cover all the reservoir.
            auxf.export(ssamasked, 'S23AQUA_' + self.parameters.date + '_' + self.parameters.grid + '_' + y_band, reference, dest)
        return None
